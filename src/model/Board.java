...
public class Board {
	...
	public String imprimir() {
		...
		String out="" ;
		int i =1;
		out=nextCol(nodo(first),i);
		 return out;
	}
	private Node nodo(Node first2) {
	...
	private String nextCol(Node first2,int i) {
		String out="";
		
			if(first2.getNext()!= null) {
				first2.setPos(i++);
				out= "["+first2.getPos()+"]";				
				out+=nextCol(first2.getNext(),i++);
				
			}if(first2.getUp()== null) {
				first2.setPos(i++);
				out= "["+first2.getPos()+"]";				
				out+=nextCol(first2.getNext(),i++);
	...
	*/
	...
	private boolean positionSnake(Node first2,int position,String let) {
		...
		if(first2.getPos()== position) {	
			...
			positionSnake(first2.getNext(), position,let);
		}
		return out;
	}
}
